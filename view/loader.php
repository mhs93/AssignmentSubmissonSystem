...
if (isset($_POST["assignment"])){
    $courseID = $_POST["assignment"];
    $sql= "SELECT students.title, students.reg_no , assignment.file FROM students LEFT JOIN assignment on students.id = 
assignment.reg_no WHERE assignment.course_id = $courseID";
    $stmt = Database::Prepare($sql);
    $stmt->execute();
    $data =  $stmt->fetchAll();
...
foreach ($data as $results) {
    $output .= '<tr>';
    $output .= '<td>' . $results['title'] . '</td>';
    $output .= '<td>' . $results['reg_no'] . '</td>';
    $output .= '<td><a href="files/' . $results['file'] . '">' . $results['file'] . '</a></td>';
}
    echo $output;
}
